...
write.table(acs,"./AnnualCountryStats.txt",sep="\t", row.names=FALSE, col.names = TRUE)
...
write.table(worksIn,"./WorksIn.txt",sep="\t", row.names=FALSE, col.names = TRUE)
...
write.table(ads,"./AnnualDemographicStats.txt",sep="\t", row.names=FALSE, col.names = TRUE)

#Year
year <- bind_rows(wages, acs, demo)
year <- unique(subset(year, select = c(year)))
year <- year[order(year$year), ]
write.table(year,"./Year.txt",sep="\t", row.names=FALSE, col.names = TRUE)

#Demographics
sex <- c("Male", "Female")
demo <- data.frame(sex)
write.table(demo,"./Demographics.txt",sep="\t", row.names=FALSE, col.names = TRUE)